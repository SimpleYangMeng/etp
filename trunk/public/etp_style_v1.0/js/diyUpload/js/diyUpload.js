...
(function( $ ) {
    ...
    $.fn.extend({
		...
		*/
        diyUpload:function( opt, serverCallBack ) {
            ...
            if( opt.cancelFileTrigger ) {
				var cancelFileTrigger = opt.cancelFileTrigger;
				delete opt.cancelFileTrigger;
			}

            if( opt.cancelAllFileTrigger ) {
				var cancelAllFileTrigger = opt.cancelAllFileTrigger;
				delete opt.cancelAllFileTrigger;
			}

            if( opt.selectFileTrigger ) {
				var selectFileTrigger = opt.selectFileTrigger;
				delete opt.selectFileTrigger;
			}

            if( opt.removeFailedWhenSelect ) {
				var removeFailedWhenSelect = opt.removeFailedWhenSelect;
				delete opt.removeFailedWhenSelect;
			}
            ...
            if( typeof opt.removeCancel != 'undefined') {
                var removeCancel = opt.removeCancel;
				delete opt.removeCancel;
            }
			...
			webUploader.on('fileQueued', function( file ) {
                ...
                //强制检查已选的文件的数量,
                var tmpState = webUploader.getStats()
                if( opt.fileNumLimit < tmpState.queueNum || opt.fileNumLimit < tmpState.successNum + 1 ) {
                    //删除最新添加的文件
                    var allFiles = webUploader.getFiles();
                    var lastFile = allFiles[allFiles.length-2];
                    removeLi( $('#fileBox_'+  lastFile.id), lastFile.id, webUploader );
                }

				createBox( $fileInput, file ,webUploader ,$fileInputId);

                if( selectFileTrigger )
			        selectFileTrigger();
                //强制清除上传失败的图片
                //if( removeFailedWhenSelect ) {
                    var errorFile = webUploader.getFiles('error');
                    $.each(errorFile,function(k,v){
			...
			webUploader.on('fileDequeued', function( file ) {
				var state = webUploader.getStats();
                if( state.queueNum == 0 ) {
                    if( cancelAllFileTrigger )
                        cancelAllFileTrigger();
                }
			});
			...
			webUploader.on('uploadFinished', function(){
                if( typeof removeCancel == 'undefined' || removeCancel == true) {
				    $fileInput.next('.parentFileBox').children('.diyButton').remove();
                } else {
                    $fileInput.next('.parentFileBox').find('.diyStart').remove();
                }
			});
			//绑定发送至服务端返回后触发事件;
			...
			webUploader.on('uploadSuccess',function( file, response ){
                if( typeof response.state != 'undefined' && response.state == 1) {
                    var $fileBox = $('#fileBox_'+file.id);
                    var $diyBar = $fileBox.find('.diyBar');	
			...
			webUploader.on('error', function( code ) {
				...
				switch( code ) {
					...
					break;
					case  'F_EXCEED_SIZE' : text = '文件大小超过限制，请不要超过300KB';
					break;
	...
	function getOption(objId) {
		...
		*/
		return {
			...
			swf: '/etp_style_v1.0/js/diyUpload/js/Uploader.swf',
			chunked: false,
			chunkSize: 512 * 1024,
			//类型限制;
			accept:{
	...
	function createBox( $fileInput, file, webUploader, $fileInputId) {
		...
		$('.diyButton').width( $('.diyButton').prev('.fileBoxUl').children('.diyUploadHover').width() );
		...
		webUploader.makeThumb( file, function( error, dataSrc ) {
			if ( !error ) {	
				$fileBox.find('.viewThumb').append('<img src="'+dataSrc+'" ><textarea name="'+$fileInputId+'[]">'+dataSrc+'</textarea>');
			}
		});	
	}
