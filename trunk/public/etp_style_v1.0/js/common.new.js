...
Roll.prototype = {
    ...
    roll: function () {
        if (this.options.direction == "up") {
            ...
            this.clone.css({"margin-top": "-" + this.boxH + "px"});
    ...
    rollUp: function () {
        ...
        this.timer = setInterval(function () {
            ...
            _this.box.animate({'marginTop': marginTop}, _this.options.rollTime, function () {
                if (util.getNumPixel(_this.box.css("marginTop")) + _this.boxH < 1) {
                    _this.box.css({'marginTop': 0});
                }
            });
    ...
    rollDown: function () {
        ...
        this.timer = setInterval(function () {
            ...
            _this.clone.animate({'marginTop': marginTop}, _this.options.rollTime, function () {
                if (util.getNumPixel(_this.clone.css("marginTop")) > -1) {
                    _this.clone.css({'marginTop': "-" + _this.boxH + "px"});
                }
            });
        }, this.options.spaceTime + this.options.rollTime);
    }
        ...
        switch (this.scrollAnimate) {
            case "opacity" :
                ...
                if (_this.bgColorSwicth) {
                    _this.bannerBox.animate({backgroundColor: _this.bgColorArr[_this.targetIndex]}, _this.bgColorSpeed);
                }
...
function Drag(options) {
    ...
    this.dragParam = {diffX: '', diffY: '', moveX: '', moveY: ''};//diffX|Y : 初始时，鼠标与被移动元素原点的距离,moveX|Y : 移动时，被移动元素定位位置 (新鼠标位置与diffX|Y的差值)
    this.init();
}
Drag.prototype = {
    init: function () {
        ...
        $(document).unbind("mousemove").mousemove(function (e) {
            if (_this.draging) {
                ...
                if (_this.direction == 'all') {
                    _this.moveElement.css({'left': _this.dragParam.moveX + 'px', 'top': _this.dragParam.moveY + 'px'});
                } else if (_this.direction == 'vertical') {
                    _this.moveElement.css({'top': _this.dragParam.moveY + 'px'});
                } else if (_this.direction == 'horizontal') {
                    _this.moveElement.css({'left': _this.dragParam.moveX + 'px'});
                }
                if (_this.afterDrag) {
                    _this.afterDrag();
                }
...
Dialog.prototype = {
    init: function () {
        var d = $(".dialog");
        if (d.length > 0 && d.find(".dialog_main").css("z-index") == this.options.zIndex) {
            this.dialog = d;
        } else {
            this.dialog = '<div class="dialog';
            if (this.options.skin) {
                this.dialog += ' ' + this.options.skin + '-skin none">';
            } else {
                this.dialog += ' skin none">';
            }
            this.dialog += '<div class="dialog_bg none"></div><div class="dialog_main"><div class="dialog_box">' +
                '<div class="dialog_top"><span class="f16 black_text dialog_title none"></span><span class="close_dialog"></span></div>' +
                '<div class="dialog_body';
            if (this.options.skin) {
                this.dialog +='">';
            }else{
                this.dialog += ' padding10">';
            }
            this.dialog +='</div><div class="dialog_bottom none"></div>' +
                '</div></div></div>';
    ...
    showDialog: function () {
        ...
        this.main.offset({top: top > 0 ? top : 0, left: left > 0 ? left : 0});
    ...
    showDrag: function () {
        if (this.options.dragAble) {
            new Drag({
                ...
                moveArea: [0, $(document).width() - this.main.width() - util.getNumPixel(this.main.css("padding")) * 2, 0, $(document).height() - this.main.height() - util.getNumPixel(this.main.css("padding")) * 2]
            });
        } else {
...
//            this.main.css({"z-index":this.options.zIndex,"padding":padding,"background-color":bgColor});
            ...
            this.main.css({"padding": 0, "z-index": this.options.zIndex});
        }
...
//        this.box.css({"background-color":bg});
        ...
        if (this.options.skin == 'blue') {
            var box_width = util.getNumPixel(this.options.width) + "px";
        }
        ...
        this.body.css({"width": this.options.width, "height": this.options.height});
        return this;
    },
    showTop: function () {
    ...
    bindEvent: function () {
        ...
        this.top.find(".close_dialog").unbind("click").click(function () {
            _this.closeDialog();
            if (_this.options.cCloseCallBack) {
                _this.options.cCloseCallBack();
            }
        });
    ...
    changeByWindow: function () {
        ...
        $(window).unbind("resize").resize(function () {
            ...
            _this.main.offset({top: top > 0 ? top : 0, left: left > 0 ? left : 0});
        });
    }
};
$.extend(Dialog, {
    ...
    info: function (options) {
        ...
        options = $.extend({showFooter: true, showCloseBtn: true, title: "提示信息"}, options);
    ...
    confirm: function (options) {
        ...
        options = $.extend({showFooter: true, showCancelBtn: true, showOKBtn: true, title: "提示信息"}, options);
        new Dialog(element, options);
    },
    /**
     * 错误提示信息，带有红叉图标
 ...
 * @constructor
 */
function NumAnimate(options) {
    this.box = options.box; //数字显示框
    this.targetNum = options.targetNum;//显示的数字
    this.animateTime = options.animateTime || 1000; //动画时间
    this.animate = options.animate || "increase";//动画效果
    this.init();
}
NumAnimate.prototype = {
    ...
    animation: function () {
        switch (this.animate) {
            case "increase":
                var _this = this;
                var point = this.targetNum.indexOf(".");//小数点的位置
                var pointAfterNum = 0;//小数点后的位数
                point > 0 && (pointAfterNum = this.targetNum.length - point - 1);
                var finalNum = Number(this.targetNum.replace(".", ""));//去除小数后的数
                var times = this.animateTime / 80 | 0;  //递增次数
                var increase = finalNum / times | 0;    //每次递增增加的数值
                var i = 0, j = 0;
                _this.timer = setInterval(function () {
                    i++;
                    j += increase;
                    _this.box.html(j / Math.pow(10, pointAfterNum));
                    if (i >= times) {
                        clearInterval(_this.timer);
                        _this.box.html(_this.targetNum);
                    }
                }, 50);
        }
    }
    ...
    showTips: function (ele) {
        ...
        switch (this.options.position) {
            case "top":
                ...
                this.wrap.find(".tips_arrow em,.tips_arrow span").css({left: (tipsWidth / 2 - 8) + "px"});
                break;
            case "bottom":
            ...
            case "right":
                ...
                this.wrap.find(".tips_arrow em,.tips_arrow span").css({top: (tipsHeight / 2 - 8) + "px"});
                break;
            case "left":
                tipsTop = top + height / 2 - tipsHeight / 2;
        ...
        this.wrap.css({top: tipsTop + 'px', left: tipsLeft + 'px'}).stop(true, true).fadeIn();
    }
};
...
Validator.prototype = {
    verify: function () {
        ...
        if (typeof this.opt.valid == "function") {//如果是方法
            this.validResult = this.opt.valid(val);
        } else if (typeof this.opt.valid == "object") {
            for (var key in this.opt.valid) {
                var valid = Validator.rules[key] || Validator.functions[key];
                if (key == "required" && this.opt.bind == "blur") {
                    //blur事件时，忽略required；
                    continue;
                }
                if (valid instanceof RegExp) {
                    if (val) {
                        if (!valid.test(val)) {
                            this.validResult = this.opt.valid[key];
                            break;
                        }
                    }
                } else {
    ...
    processResult: function () {
        if (this.validResult != true) {
            this.err.css("color", "#ff4d4d");
            this.err.html(this.validResult);
            if (this.border) {
                this.elem.parents(this.border).css("border-color", "red");
            } else {
                this.elem.css("border-color", "red");
            }
        } else {
            this.err.css("color", "#555");
            this.err.html("");
            if (this.border) {
                this.elem.parents(this.border).css("border-color", "#ccc");
            } else {
                this.elem.css("border-color", "#ccc");
            }
        }
    }
};
$.extend(Validator, {
    ...
    rules: {
        string: /^[a-zA-Z]+$/,
        phone: /^(13|15|18|14|17)[0-9]{9}$/,
        num: /^\d+$/,
        pic_code: /^[0-9]{4}$/,
        phone_code: /^[0-9]{6}$/,
        goods_url: /^((http|https|ftp)\:\/\/)?([a-zA-Z]+)(\.[a-zA-Z]+)*(\/[\w_-]+)*\/([\w_-]+(\.[a-zA-Z])?)(\S)*$/,
        money: /^[0-9][\d]*(,\d{3})*(\.\d{0,2})?$/,
        price: /^[1-9][\d]*(,\d{3})*(\.\d{0,2})?$/,
        email: /^(\w)+(\.\w+)*@(\w)+((\.\w+)+)$/,
        referrer: /^[0-9a-zA-Z]{7}$/
    },
    functions: {
        ...
        newphone: function (value) {
            var old_phone = $("input[name='user_tel']").val().trim();
            if (old_phone == value) {
                return false;
            }
            return true;
        ...
        password: function (value) {
            if (value) {
                if (/^[\@A-Za-z0-9\!\#\$\%\^\&\*\.\~]{8,16}$/.test(value) && /[a-zA-Z]+/.test(value) && /[0-9]+/.test(value)) {
                    if (getLen(value) < 8 || getLen(value) > 16) {
                        return false;
                    }
                    return true;
                } else {
        ...
        id_card: function isIdCardNo(code) {
            if ($.trim(code) == "") {	//不做空校验
                return true;
            }
            if (code.length != 18) {	//支持15或18位长度
                return false;
            }
            // 地区校验
            var area = {
...
function FormCheck(form, options) {
    this.form = form;
    var defaults = {
        submit: ".submit",     //提交按钮选择器
        phone_btn: ".phone_verify",    //短信验证码选择器
        img_code_btn: "#imgcode,#changeImgCode",   //图片验证码选择器
        compatibility: false,  //IE6-9对placeholder属性做兼容处理,IE6-IE9,将placeholder转化为value
        border: "",            //input输入框的边框（用于颜色变化），默认是本身，可以为input框父元素
        tips: "",              //校验信息显示框，可为空。默认为 $("."+input.attr("id")+"_tips").
        showEle: ".input_box", //点击输入框时，小提示显示的位置依赖元素
        position: "right",     //提示框显示的地方
        enterSub: false,       //enter建提交表单
        beforeSubmit: null,    //提交表单前的判断
        fields: []
    };
    this.options = $.extend(defaults, options);
    this.tips = this.options.tips ? $(this.options.tips) : "";
    this.init();
    this.bindEvent();
}
...
FormCheck.prototype = {
    init: function () {
        ...
        if (this.options.fields.length == 0) {
            for (var i = 0; i < inputs.length; i++) {
                var input = $(inputs[i]);
                var tips = $("." + input.attr("id") + "_tips");
                var field = {
                    item: input,
                    tips: tips.length > 0 ? tips : "",
                    verify: input.attr("verify")
                };
                this.options.fields.push(field);
                if (this.options.compatibility && client.browser.ie > 0 && client.browser.ie < 10) {
    ...
    bindEvent: function () {
        var form = this.form;
        var _this = this;
        new Tip(form.find("input"), {
            eventType: "focus",
            position: this.options.position,
            showEle: this.options.showEle
        });
        if (this.options.enterSub) {
            $(document).keydown(function (e) {
                if (e.keyCode == 13) {
                    form.find(_this.options.submit).trigger('click');
                }
            })
        }
        form.find("input").focus(function () {
            ...
            if (_this.options.border) {
                $(this).parents(_this.options.border).css("border-color", "#43C9BE");
            } else {
                $(this).css("border-color", "#43C9BE");
            }
            $(this).prop("place", $(this).attr("placeholder"));
            $(this).prop("placeholder", "");
            if (!_this.tips) {
                form.find("." + $(this).attr("id") + "_tips").html("");
            }
        }).blur(function () {
            $(this).prop("placeholder", $(this).prop("place"));


            if (_this.options.compatibility && client.browser.ie > 0 && client.browser.ie < 10) {
                //增加条件判断 如果输入值不等于Placeholder 则不改变值为placeholder值 开始
                if ($(this).val() == "") {
...
//                增加条件判断结束
            ...
            if (!$(this).attr("verify")) {
                if (_this.options.border) {
                    $(this).parents(_this.options.border).css("border-color", "#ccc");
                } else {
                    $(this).css("border-color", "#ccc");
                }
            }
        });
        form.find(this.options.img_code_btn).click(function () {
            form.find("img").attr("src", "/member/imgverify?amp;ts=" + Math.random());
        });
        form.find(this.options.phone_btn).click(function () {
            var btn = $(this);
            sendTelCode(btn);
        });
        form.find(this.options.submit).click(function () {
            var protocol = form.find("input[verify='protocol']");
            if (protocol.length > 0) {
                var tips = _this.tips || form.find("." + $(protocol).attr("id") + "_tips");
            ...
            if (_this.options.beforeSubmit) {
                if (!_this.options.beforeSubmit()) {
                    return false;
                }
            }
            if (_this.validator("show")) {
                if (form.attr("submit") && form.attr("submit") == "true") {
                    form.submit();
                } else {
                    $.ajax({
                        ...
                        success: function (data) {
                            if ("1" == data.ret) {
                                if (data.url) {
                                    ...
                                    //成功不跳转页面
                                    new Dialog(data.msg, {height: "70px"});
                                }
                            } else {
                                if (data.url) {
                                    location.href = data.url;
                        ...
                        error: function (data) {
                            new Dialog("系统异常，请稍后重试，或联系客服人员。", {height: "70px"});
                        }
                    });
                }
        ...
        function sendTelCode() {
            var btn = form.find(_this.options.phone_btn);
            var tips = _this.tips;
            var user_v_code_ele = form.find("input[name='user_v_code']");
            var user_tel_ele = form.find("input[name='user_tel']");
            var validResult = true;
            var data = "";
            //修改手机发送短信校验码
            var user_new_tel = form.find("input[name='user_new_tel']");
            if (user_new_tel.length > 0) {
                validResult = validResult && new Validator(user_new_tel, {
                        bind: "show",
                        valid: {
                            required: "新手机号码不能为空",
                            phone: "新手机号码格式不正确",
                            newphone: "新手机号码不能和旧手机号码一样"
                        },
                        err: tips
                    }).validResult;
                if (validResult == true) {
                    if (tips) {
                        ...
                        form.find(".user_tel_code_tips").html("");
                    }
                    data = "user_new_tel=" + user_new_tel.val();
                    $.ajax({
                        type: "post",
                        url: btn.attr("url"),
                        data: data,
                        ...
                        success: function (data) {
                            if ("1" == data.ret) {
                                ...
                                if (tips) {
                                    ...
                                    form.find(".user_tel_code_tips").html(data.msg);
                                }
                            }
                        },
                        error: function (data) {
            ...
            //修改手机发送短信校验码 end
            if (user_v_code_ele.length > 0) {
                validResult = validResult && new Validator(user_v_code_ele, {
                        bind: "show",
                        valid: {
                            required: "图片验证码不能为空",
                            pic_code: "图片验证码格式不正确"
            ...
            validResult = validResult && new Validator(user_tel_ele, {
                    bind: "show",
                    valid: {
                        required: "手机号码不能为空",
                        phone: "手机号码格式不正确"
            ...
            if (validResult == true) {
                data = "user_tel=" + user_tel_ele.val();
                if (user_v_code_ele.length > 0) {
                    data += "&user_v_code=" + user_v_code_ele.val();
                }
                $.ajax({
                    type: "post",
                    url: btn.attr("url"),
    ...
    validator: function (action) {
        ...
        for (var i = 0; i < fields.length; i++) {
            var field = fields[i];
            var verify = field.verify;
            var input = field.item;
            var valid = null;
            var tips = this.tips || field.tips;
            switch (verify) {
                case "pic_code":
                    valid = {
                        required: "图片验证码不能为空",
                ...
                case "email" :
                    ...
                    break;
                case "college" :
                    valid = {required: "所在大学不能为空"};
                    break;
                case "dorm" :
                    valid = {required: "宿舍地址不能为空"};
                    break;
                default :
                    break;
            }
            if (valid) {
                var v = new Validator(input, {
                    bind: action,
                    valid: valid,
                    err: tips,
        ...
        for (var j = 0; j < validArr.length; j++) {
            if (validArr[j].validResult != true) {
                return false;
            }
        }
        return true;
