<?php
class Table_SellerWithdraw
{
    protected $_table = null;

    public function __construct()
    {
        $this->_table = new DbTable_SellerWithdraw();
    }
     ...
     * @return mixed
     */
    public function update($row, $value, $field = "seller_withdraw_id")
    {
     ...
     * @return mixed
     */
    public function delete($value, $field = "seller_withdraw_id")
    {
        $where = $this->_table->getAdapter()->quoteInto("{$field}= ?", $value);
     ...
     * @return mixed
     */
    public function getByField($value, $field = 'seller_withdraw_id', $colums = "*")
    {
    ...
    public function getByCondition($condition = array(), $type = '*', $pageSize = 0, $page = 1, $orderBy = "")
    {
        ...
        if( isset($condition["status"]) && $condition["status"] !== ""){
            if( is_scalar( $condition["status"] ) ) {
                $select->where("status = ?",$condition["status"]);
            } else if( is_array( $condition["status"] ) && !empty( $condition["status"] ) ) {
