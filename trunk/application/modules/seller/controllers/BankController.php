 ...
 * @date:2016-10-27
 */
class Seller_BankController extends Ec_Controller_Action
{
    ...
    public function preDispatch()
    {
        ...
        $this->tplDirectory     = "seller/views/bank/";
    ...
    public function cardAction() {
        ...
        $companyRow = Service_Seller::getByField($this->_customerAuth['account_id'], 'seller_id', array('company_name'));
        $this->view->companyName = $companyRow['company_name'];
    ...
    public function addBankCardAction() {
        ...
        //获取证件类型
        $idCardType = Common_DataCache::getIdCardType( 'ON', 2, true,'asc',1);
    ...
    public function addNewOneAction() {
        ...
        foreach( $accountRows as $value ) {
            ...
            $bankAccountRow = Service_PaJzbBankCard::getByCondition( array( 'account_no' => $value['account_no'] ) );
            $accountNoId[$value['account_no']] = $value['account_id'];
            $total += count( $bankAccountRow );
            if( $total >= 2 ) {
                $return['error'] = array( array( 'errorCode' => Common_EtpCommon::$defaultErrorCode,'errorMsg' => Ec_Lang::getInstance()->getTranslate('beyondAmount') ) );
                die(Zend_Json::encode($return));
            }
            $accountNo[] = $value['account_no'];
        }
        ...
        $postArr = array(
            ...
            'super_bank_code'                   => trim( $this->_request->getParam( 'sBankNo', '') ),
            'bank_attr'                          => $bankAttr,
        );
    ...
    public function applySubAccountAction() {
        if( $this->_request->isPost() ) {
            ...
            $account = Service_Seller::getByField( $this->_customerAuth['account_id'], 'seller_id' );
            $postArr = array(
                'seller_code'                       => $this->_customerAuth['account_code'],
                'account_nick_name'                => $account['company_name'],
    ...
    public function doConfirmCardAction() {
        ...
        if( $bankCardId === '' || $accountId === '' ) {
            ...
            $account = Service_PaJzbAccount::getByField( $accountId, 'account_id' );
            if( empty( $account ) || $account['seller_id'] !== $this->_customerAuth['account_id'] ) {
                $return['error'][] = array('errorCode'=>Common_EtpCommon::$defaultErrorCode,'errorMsg'=>'银行卡不存在');
            } else {
                $card = Service_PaJzbBankCard::getByCondition( array( 'account_no' => $account['account_no'],'bank_card_id' => $bankCardId ));
                if( empty( $card ) ) {
                    $return['error'][] = array('errorCode'=>Common_EtpCommon::$defaultErrorCode,'errorMsg'=>'银行卡不存在');
        ...
        $obj = new Api_Seller( array() );
        $return = $obj->confirmBankCard($postArr,$maps);
        die( json_encode( $return ) );
    }
    ...
    public function editBankCardAction() {
        ...
        $this->view->idCardType = $idCardType;
        echo Ec::renderTpl(
            $this->tplDirectory . 'edit-bank-account.tpl',
            $this->_layoutFile,
            '我的ETP',
            null,
            null,
