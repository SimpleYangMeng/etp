 ...
 * @date:2016-10-27
 */
class Portal_PurchaserPayOrderRecordController extends Ec_Controller_Action
{
    ...
    public function preDispatch()
    {
        ...
        $this->tplDirectory     = "portal/views/purchaser_pay_order_record/";
    ...
    public function payOrderListAction(){
        ...
        if ($this->_request->isPost()) {
            ...
            if($count){
                ...
                $rows = Service_OrderPay::getByCondition($condition,$showFields, $pageSize, $page, array('order_pay_id desc'));
                $currency =  Common_DataCache::getCurrencyMaps();
                foreach( $rows as $key=>$value ) {
                    $rows[$key]['pay_currency'] = $currency[$value['pay_currency']]['currency_symbol_left'];
                }
                $return['data'] = $rows;
                $return['state'] = 1;
